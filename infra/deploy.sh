#!/usr/bin/env bash
# infra/deploy.sh — pull main, sync deps, run migrations, restart the API.
#
...
"${HOME}/.local/bin/uv" run alembic -c api/alembic.ini upgrade head

echo "==> [4/4] reload systemd unit"
