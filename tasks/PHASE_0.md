...
- [ ] Confirm `supabase db push` applies migrations cleanly to the linked Supabase project. The engine in `api/db/__init__.py` auto-adds `ssl=require` for non-localhost hosts; no code change needed.
